...
export default {
  data() {
    return {
      location: "English", // Holds the selected language
      languages: ["English", "Arabic"], // Language options
      test: "",
      props: ["text"],
      profile: false,
      Main: [],
  ...
  methods: {
    ...
    changeLanguage(lang) {
      this.location = lang; // Update the selected language
      this.$i18n.locale = lang; // Change the locale for i18n
    },
    opencc(flo) {
      console.log(flo);
...
<template>
  <div class="closer" :class="[Closer ? '' : 'active']">
    <h4>
      <a @click="closeCloser">
  ...
  <nav>
    <header>
      ...
      <v-menu transition="slide-y-transition" class="LN">
        <template v-slot:activator="{ props }">
          <v-btn v-bind="props" variant="text"
            ><v-icon>mdi-chevron-down</v-icon> {{ location }}
          </v-btn>
        </template>
        <v-list>
          ...
          <v-list-item @click="changeLanguage('Arabic')">
    ...
    <main>
      ...
      <div class="menu">
        <router-link to="/Promotions"> الصفاحات </router-link>

        <router-link to="/Promotions"> المنزل </router-link>
        <router-link to="/Promotions"> المنتجات </router-link>
